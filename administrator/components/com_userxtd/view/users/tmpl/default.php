...
<div id="userxtd" class="windwalker users tablelist row-fluid">
	<form action="<?php echo JURI::getInstance(); ?>" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data">
		...
		<?php endif;?>
				...
				<div class="btn-group pull-right">
					...
					<select id="UserXTD_field_name" class="UserXTD_field_name input-xxlarge" name="fields[]" multiple="true">
						<?php foreach($this->data->fields as $field ): ?>
							<option value="<?php echo $field->name; ?>" <?php echo (in_array($field->name, $this->data->keys)) ? 'selected' : ''; ?>><?php echo $field->title; ?> (<?php echo $field->label; ?>)</option>
						<?php endforeach; ?>
					</select>
				</div>
			</div>
			...
			<?php echo with(new FileLayout('joomla.batchtools.modal'))->render(array('view' => $this->data, 'task_prefix' => 'users.')); ?>
